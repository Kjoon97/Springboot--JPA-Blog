...
<div class="container">
	...
	<div class ="card">
	   ...
	   <c:forEach var="reply" items="${board.replys}"> <%--BoarController에서 받은 board안에 replys가 있음. replys를 reply변수 에 한 개씩 차례로 넣는다.--%>
	       ...
	       <li id ="reply-${reply.id}" class="list-group-item d-flex justify-content-between"> <%-- d-flex를 해주면 아래 2개의 div태그가 따로 구분되서 각자의 영역이된다->세로에서 가로축으로 나란히 됨.--%>
	          <div>${reply.content}</div>         <%--댓글 내용--%>       <%-- justify-content-between해주면 아래의 2개의 div가 서로떨어짐.--%>
	          <div class="d-flex">
	              <div class="font-italic">작성자: ${reply.user.username } &nbsp;</div>
	              
	              <c:if test="${reply.user.id == principal.user.id}">   <%--본인이 쓴 댓글만 삭제 버튼이 구현이 되도록함.--%> 
	              <button onClick="index.replyDelete(${board.id},${reply.id})" class="badge">삭제</button> <%--index에 replyDelte함수 어떤게시물, 어떤 댓글 id를 넘길것임--%>
	              	</c:if>
	          </div>
	        </li>
